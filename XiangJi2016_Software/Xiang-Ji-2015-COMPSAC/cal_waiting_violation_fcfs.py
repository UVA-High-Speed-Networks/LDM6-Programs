##Xiang Ji modify Yicheng's code to calculate the total delay and mean throughput for products
##
...
for Waiting_time in my_range(0.1, 1.5, 0.1):  # waiting time range 15-20, step is 0.5
...
##            print throughput
	...
	wait_violation = number_violation / float(product_num)
	print 'violation:' +str(wait_violation)
	writer.writerows([(Waiting_time * 1000, rate * 8 / 1000000, Buffer_size, utilization, mean_throughput)])
##    waiting_time_output = feedtype + '_waiting_time_' + str(Waiting_time) + '.csv'
##    writer2 = csv.writer(open(waiting_time_output, 'wb', buffering=0))
##    writer2.writerows([('Waiting_times', '')])
